...
const ImagePlaceholder = () => (
    <div className=" mt-8 w-full aspect-square bg-zinc-900/50 rounded-2xl flex items-center justify-center border border-zinc-800">
        <div className="text-center text-zinc-500">
            ...
            <p className="mt-2 text-sm font-raneva">Your generated image will appear here</p>
        </div>
    </div>
);
        ...
        <>
            ...
            <div className="container mx-auto px-4 py-8 md:py-16 max-w-2xl">
                <header className="text-center mb-10">
                    <h1 className="font-raneva text-5xl md:text-6xl font-bold text-lime-400 tracking-wider">Image Weaver</h1>
                    <p className="font-raneva-italic text-lg md:text-xl text-gray-500 mt-2">Bring your words to life</p>
                </header>

                <main>
                    <div className="bg-zinc-900/50 rounded-2xl p-6 shadow-lg backdrop-blur-sm border border-zinc-800">
                        <div className="w-full mb-4">
                            <label htmlFor="prompt-input" className="flex items-center mb-2 text-sm font-medium text-gray-300">
                                <i className="ri-keyboard-box-line mr-2 text-lg"></i>Enter your prompt
                            </label>
                            <textarea
                                id="prompt-input"
                                rows="4"
                        ...
                        <div className="w-full mb-4 relative" ref={dropdownRef}>
                            <label className="flex items-center mb-2 text-sm font-medium text-gray-300">
                                <i className="ri-list-settings-line mr-2 text-lg"></i>Choose a model
                            </label>
                            <button
                                type="button"
                            ...
                            >
                                {model}
                                <i className={`ri-arrow-down-s-line transition-transform duration-200 ${isDropdownOpen ? 'transform rotate-180' : ''}`}></i>
                            </button>
                            {isDropdownOpen && (
                    ...
                    {isLoading || error || imageUrl ? (
                        <div className="mt-8 w-full aspect-square bg-zinc-900/50 rounded-2xl flex items-center justify-center border border-zinc-800">
                            ...
                            {imageUrl && <img src={imageUrl} alt="Generated art" className="rounded-lg max-w-full max-h-full" />}
                        </div>
                    ) : (
                        <ImagePlaceholder />
                    )}
